    ...
    public class CollectableLerpMovementCommand
    {
        ...
        public void Execute(ref Transform playerManager)
        {
            if (_collectableList.Count > 0)
            {
                ...
                var directY = Mathf.Lerp(_collectableList[0].transform.localPosition.y, playerManager.position.y, 1);
                ...
                for (var i = 1; i < _collectableList.Count; i++)
                {
                    ...
                    directX = Mathf.Lerp(_collectableList[i].transform.localPosition.x, pos.x, _stackData.StackLerpXDelay);
                    directY = Mathf.Lerp(_collectableList[i].transform.localPosition.y, pos.y, _stackData.StackLerpYDelay);
                    ...
                    _collectableList[i].transform.LookAt(new Vector3(pos.x, _collectableList[i].transform.position.y, pos.z));
                }
            }
